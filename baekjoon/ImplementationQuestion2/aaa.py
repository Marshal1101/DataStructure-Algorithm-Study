...
class Monomino:
    ...
    # 터진 라인 위쪽 연결된 블록 체크
    def __connect(self, bi:int, idx:int, i:int, visited:set):
        vist = set()
        mh = self.height[i]
        stk = [(idx, i)]
        vist.add((idx, i))
        while stk:
            ...
            for dx, dy in [(0, 1), (1, 0), (0, -1), (-1, 0)]:
                nidx = cidx + dx
                ni = ci + dy
                if nidx == bi or nidx < 0 or nidx == self.len or ni < 0 or ni == self.width \
                    or self.lines[nidx][ni] == 0:
                    continue
                if not (nidx, ni) in vist and not (nidx, ni) in visited:
                    vist.add((nidx, ni))
                    stk.append((nidx, ni))
        tmp = []
        for cidx, ci in vist:
            if cidx == idx:
                if mh < self.height[ci]:
                    mh = self.height[ci]
                tmp.append(ci)
        for ci in tmp:
            self.height[ci] = mh
        visited.update(vist)
        # print(self.height, bi, "conec", idx, i, tmp)
    ...
    def check(self):
        ...
        while self.__check_full():
            ...
            while h < self.len:
                ...
                for i in range(self.width):
                    if self.lines[idx][i]:
                        if not (idx, i) in visited:
                            self.__connect(bi, idx, i, visited)
                        ci.append(i)
                    elif ci:
