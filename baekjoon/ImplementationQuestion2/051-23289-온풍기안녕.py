...
def get_wind_diff(wind_diff: dict, pos: tuple, delta: list[tuple], board: list[list], wall: set) -> None:
    i, j, d = pos
    i = i + delta[d][0]
    j = j + delta[d][1]
    if (i, j) in wind_diff:
...
def spread(R: int, C: int, delta: list[tuple], board: list[list], wall: set) -> None:
    visited = set()
    diffusion = []
